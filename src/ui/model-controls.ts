...
import { getActiveModelProfile, type ModelProfile } from '../settings/models';
 ...
 * 避免清空用户已输入的内容；徽章文本由 onActiveChange 回调同步更新。
 * @param container 父容器（面板头部）
 * @param plugin 插件实例
 * @param options.onActiveChange 激活档位变化回调
 * @returns 快速切换控件控制器
 */
export function createModelQuickSelect(
	container: HTMLElement,
	plugin: EnPracticePlugin,
	options: { onActiveChange?: (profile: ModelProfile) => void } = {},
	...
	select.addEventListener('change', () => {
		...
		void plugin.saveData(plugin.settings).then(() => {
			options.onActiveChange?.(profile);
