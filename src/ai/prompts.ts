...
JSON 示例：
{
  ...
  "targetWords": ["improve", "efficiency"]
}
...
- 生词本目标单词：学习者指定必须练到的英文单词及其中文含义，每行格式“- 单词：含义”。
  - 提供了目标单词时，生成的中文语句必须自然包含每个单词对应的中文含义，使学习者把中文语句翻译成英语时必须用到这些英文单词；允许按语法需要做合理的词形变化（时态、单复数、词性转换等）。
  - 目标单词与其他输入同时提供时：内容服从主题，语法结构借鉴参考表达，且全部目标单词必须出现在最终的英语翻译中，三者分别约束内容、形式与词汇，互不冲突。
  - 未提供目标单词时忽略本项。

字段要求：
- hint：提示信息，围绕目标语法点或难点表达给出提醒，不要直接泄露完整译法。
- targetGrammar：说明本句翻译成英语时应使用的语法点；若提供了参考英语表达，需说明借鉴了参考表达中的哪些结构。
- targetWords：把本句翻译成英语时必须用到的英文单词列表（使用用户提供的单词拼写）；未提供生词本目标单词时省略该字段。
